...
const updateNumbers = (obj) => {
  let arr = [];
  Object.entries(obj).forEach(item => arr.push(item.join(': ')));
  return arr;
};
...
const hasChildrenValues = (arr, character) => {
  ...
  arr.forEach(item => {
    if((item.name === character || item.spouse === character) && Object.values(item)[2]) hasChildern = true;
  });
  return hasChildern;
};
...
const hasChildrenEntries = (arr, character) => {
  ...
  arr.forEach(item => {
    if((Object.entries(item)[0][1] === character || Object.entries(item)[1][1] === character) && Object.entries(item)[2][1].length > 0) hasChildren = true;
  });
  return hasChildren;
};
...
const totalCharacters = (arr) => {
  ...
  arr.forEach(item => {
    totalCharacters += 1 + item.children.length;
    item.spouse && totalCharacters ++;
  });
...
const houseSize = (arr) => {
  const sizes = [];
  arr.forEach(item => sizes.push({
    house: item.house,
    members: item.spouse ? 2 + item.children.length : 1 + item.children.length
  }));
  return sizes;
};
...
const houseSurvivors = (arr) => {
  ...
  arr.forEach(item => survivors.push({
    house: item.house,
    members: item.spouse && !deceasedSpouses.includes(item.spouse) ? 2 + item.children.length : 1 + item.children.length
  }));
  return survivors;
};
