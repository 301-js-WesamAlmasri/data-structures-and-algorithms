...
const splitFoods = (recipe) => {
  let result = [];
  recipe.ingredients.forEach(item => {
    result.push(item.split(' ').slice(2, item.length - 1).join(' '));
  });
  return result;
};
...
const removeEvenValues = (arr) => {
  ...
  for(let i = 0; i < length; i++){
    if(arr[i - itemsRemoved] % 2 === 0){
      arr.splice(i - itemsRemoved, 1);
      itemsRemoved ++;
    }
  }
};
...
const removeVowels = (str) => {
  ...
  str.split('').forEach(item => {
    if(!vowels.includes(item)) newStrArr.push(item);
  });
...
const extractVowels = (str) => {
  ...
  str.split('').forEach(item => {
    if(!vowels.includes(item)) {
      newStrArr.push(item);
    }else vowlesFouned.push(item);
  });
  vowlesFouned.sort((a, b) => {
    if(a < b) return -1;
    else if(a > b) return 1;
    else return 0;
  });
  return [newStrArr.join(''), vowlesFouned.join('')];
};
