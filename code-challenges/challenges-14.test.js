...
const screenForNames = (arr) => {
  return arr.filter(item => /^((Mr|Mrs|Ms|Dr).\s)[a-zA-z]+/.test(item));
}
...
------------------------------------------------------------------------------------------------ */
const isSecure = (url) => {
  return /^https:\/\/(www.)?[a-z]+.com/i.test(url);
};
...
const helpCheck = (val1, val2, val3) => {
  return !(val1 === '' && val2 === '' && val3 === '') && (val1 === val2 && val1 === val3);
};
...
const detectTicTacToeWin = (board) => {
  ...
  //check diag
  if (helpCheck(board[0][0], board[1][1], board[2][2]) || helpCheck(board[0][2], board[1][1], board[2][0])) return true;
