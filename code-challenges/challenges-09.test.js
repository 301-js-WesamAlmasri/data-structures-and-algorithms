...
function transformToLis(obj){
  return Object.entries(obj).reduce((resultArr, item) => {
    resultArr.push(`<li>${item[0]}: ${item[1]}</li>`);
    return resultArr;
  } ,[]);
}
...
const countNumberOfElements = (arr) => {
  return arr.reduce((total, _) => total = total + 1, 0);
};
...
const returnNames = (arr) => {
  return arr .reduce((resultArr, character) => {
...
const countNumberOfChildren = (arr) => {
  return arr.reduce((totalNumberOfChildren, character) => {
    totalNumberOfChildren += character.children ? character.children.length : 0;
...
const calculateAverage = (arr) => {
  let accumulator = arr.reduce((acc, number) => {
...
const countPrimeNumbers = (arr) => {
  return arr.reduce((primeEl, number) => isPrime(number) ? primeEl +=1 : primeEl, 0);
};
...
const extractStat = (statName, arr) => {
  return arr.reduce((target, stat) => stat.stat.name === statName ? stat: target, null);
};
...
const extractChildren = (arr) => {
  return arr.filter(character => character.name.includes('a')).reduce((childrenNames, character) => character.children ? childrenNames.concat(character.children) : childrenNames,[]);
};
