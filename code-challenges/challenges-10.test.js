...
const count = (target, input) => {
  return input.reduce((result, arr) => {
    let repetition  = arr.reduce((acc, number) => target === number ? acc += 1 : acc ,0);
...
const totalSum = (input) => {
  let arrayOfSums = input.map(arr => {
    let sum = arr.reduce((sum, number) => sum + number, 0);
    return sum;
  });

  let result = arrayOfSums.reduce((total, sum) => total + sum, 0);
  return result;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(arr => {
    let newArrAfterRemoving = arr.filter(element => typeof element === 'number' && element % 5 === 0);
    let newArrAfterRaisingPower = newArrAfterRemoving.map(number => Math.pow(2, number));
    return newArrAfterRaisingPower;
  });
};
...
let findMaleAndFemale = (data) => {
  let result = data.reduce((names, character) => {
...
let findShortest = (data) => {
  let shortestCharacterObj = data.reduce((shortestCharacter, character) => {
    if(parseInt(character.height) < parseInt(shortestCharacter.height)) return character;
    else return shortestCharacter;
  }, data[0]);

  return shortestCharacterObj.name;
};
