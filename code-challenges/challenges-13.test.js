...
const longestString = (arr) => {
  ...
  let lengthOfStrings = arr.map((str, index) => [str.length, index]);
  let maxStr = lengthOfStrings.reduce((maxStr, currentStr ) => {
    return currentStr[0] > maxStr[0] ? currentStr : maxStr;
  },lengthOfStrings[0]);
...
const findEvery = (arr, target) => {
  return arr.reduce((bool, str) => bool && str.includes(target), true);
};
...
const sortByDay = (arr) => {
  return daysOfWeek.map(day => {
    let daysEvents = [];
    arr.forEach(event => {
      if(event.includes(day)) daysEvents.push(event);
    });
    return daysEvents;
  });
};
